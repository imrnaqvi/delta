-- 082_asset_dctnry_gic_and_pic_change.sql
-- Create a ASSET_DCTNRY  GIC_CD and PIC_CD change event and execute it through md_rule_executor_pkg.execute_run.
-- Assumes metadata for rule_id = 163 and 164 already exists.

set serveroutput on

prompt Running ASSET_DCTNRY GIC_CD PIC_CD change event through execute_run...

declare
  l_rule_id            number := 163;
  ...
  l_new_gic_cd         varchar2(100) := '45201020';
   l_old_pic_cd         varchar2(100) := '70';
  l_new_pic_cd         varchar2(100) := '75';
...
begin
  ...
  insert into md_run (
    ...
    'SELECTIVE',
    'RUNNING',
    'gic_cd_pic_cd_change_event_script',
    '{"scenario":"asset_dctnry_gic_cd_pic_cd_change"}'
  ...
  insert into md_change_event (
    ...
    'ASSET_DCTNRY',
    '{"ASSET_ID":"' || l_asset_id || '"}',
    'ASSET_DCTNRY_' || l_asset_id,
    systimestamp,
    'ASSET_DCTNRY_GIC_CD_PIC_CD_EVT_' || l_evt_suffix,
    'NEW'
  ...
  dbms_output.put_line('asset_dctnry_gic_cd_pic_cd_change_event_script COMPLETED');
exception
  when others then
    dbms_output.put_line('asset_dctnry_gic_cd_pic_cd_change_event_script FAILED: ' || sqlerrm);
    raise;
end;
/

prompt ASSET_DCTNRY GIC_CD PIC_CD change event script complete.
