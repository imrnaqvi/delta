-- 077_asset_dctnry_gic_cd_change.sql
-- Create a ASSET_DCTNRY  GIC_CD change event and execute it through md_rule_executor_pkg.execute_run.
-- Assumes metadata for rule_id = 163 already exists.

set serveroutput on

prompt Running ASSET_DCTNRY GIC_CD change event through execute_run...
...
begin
  ...
  insert into md_run (
    ...
    '{"scenario":"asset_dctnry_gic_cd_change"}'
  ...
  insert into md_change_event (
    ...
    systimestamp,
    'ASSET_DCTNRY_GIC_CD_EVT_' || l_evt_suffix,
    'NEW'
  ...
  dbms_output.put_line('asset_dctnry_gic_cd_change_event_script COMPLETED');
exception
  when others then
    dbms_output.put_line('asset_dctnry_gic_cd_change_event_script FAILED: ' || sqlerrm);
    raise;
end;
/

prompt ASSET_DCTNRY GIC_CD change event script complete.
