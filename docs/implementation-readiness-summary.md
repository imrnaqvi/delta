...
- Coverage exists for selector, cross-entity context, runtime parameters, target DML, gate evaluation, expression validator, and expression function registry via smoke scripts 060-069.
...
|---|---|---|---|---|---|---|
| 1 | Replace literal-concatenated target DML with bind-oriented execution in apply_target_actions | High | Medium | High | plsql/packages/md_rule_executor_pkg.pkb | No dedicated SQL-injection hardening test |
...
| 4 | Add strict diagnostics schema contract for md_impact_trace payloads | Medium | Low | High | plsql/packages/md_rule_executor_pkg.pkb, plsql/packages/md_source_context_resolver_pkg.pkb | No payload-schema validation test |
...
1. Implement bind-oriented target DML execution path and regression-test with 066.
...
- Prioritize SQL safety and consistency (binds + substitution unification).
...
## Evidence References
- plsql/packages/md_rule_executor_pkg.pkb :: execute_run, apply_target_actions, log_output_eval_failure_trace
